...
|---|---|---|
| 2026-09-14 | — | Topia-style top-down rewrite: replaced isometric renderer with GPU-batched Phaser Tilemap (12-type tileset, canvas-generated). Ground baked once. All 45 buildings + trees baked into one RenderTexture. Player switched from per-frame Graphics redraw to pre-generated direction textures (8 sprites, setTexture() per frame). ~100+ draw calls/frame → ~5. Map HUD coordinate system updated iso→top-down (TW-based). Camera zoom 2.2×. |
| 2026-09-14 | — | Full city expansion: 48×48 iso grid (was 24×24). New districts: Marina (dock tiles + boats up to 50p), Financial District (D/P tiles, outdoor tables 8p), Hospital + conf room, Library 1 + Library 2, Central Park with lake + 9 benches (8p each), North Park, Ranch houses (north + south), Commerce District, Mall, City Hall, University, Gym. Spatial-audio seat kinds: bench/boat/table. Map HUD updated: 48×48 RAW grid, dock+lake Cell types, MARKERS/WAYPOINTS/labels/road-lines synced to new layout. |
